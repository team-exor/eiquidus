...
const market_url_template = 'https://www.exbitron.com/trading/{coin.toLowerCase()}{base.toLowerCase()}';
...
function get_chartdata(coin, exchange, cb) {
  ...
  var req_url = base_url + '/public/markets/' + coin.toLowerCase() + exchange.toLowerCase() + '/k-line?period=60&limit=100';
  ...
  request({ uri: req_url, json: true, headers: {'User-Agent': 'bitweb'} }, function (error, response, chartdata) {
    if (error)
      ...
      for (var i = 0; i < chartdata.length; i++){
       ...
       // }
        processed.push([parseInt(chartdata[i][0])*1000, parseFloat(chartdata[i][1]), parseFloat(chartdata[i][2]), parseFloat(chartdata[i][3]), parseFloat(chartdata[i][4])]);
    }
