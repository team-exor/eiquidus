  ...
  request({uri: url, json: true, headers: {'User-Agent': 'bitweb'}}, function (error, response, body) {
    if (error)
      ...
      var ticker = body.ticker;
            ...
            summary['change'] = ticker.price_change_percent;
...
function get_trades(coin, exchange, cb) {
  var req_url = base_url + '/api/v2/peatio/public/markets/' + coin.toLowerCase() + exchange.toLowerCase() + '/trades';
...
function get_chartdata(coin, exchange, cb) {
  ...
  var req_url = base_url + '/public/markets/' + coin.toLowerCase() + exchange.toLowerCase() + '/k-line?period=30&limit=100';
